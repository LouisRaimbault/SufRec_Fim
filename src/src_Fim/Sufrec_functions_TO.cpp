...
void init_Rnodes_TO (rnodes* outrnodes, int* ngroot ,int* vecsom,  int* ind ,int * mod,uint64_t** Bitdata, int nbleft, int nbcases,  int reste, int * nb_freq_)
{ 
  auto * yb = &Ultab_TO[0]; uint64_t u; int t = 0; int j,nbc;
  auto * yd = &Ultab_TO[64]; auto * ybi = &Ultab_TO[0]; uint64_t * ptul;
  int sum = 0; 
  if (reste ==0)
    { nbc = nbcases;
  ...
  for (auto i=0; i < nbleft;i++)
    { sum = vecsom[i];
      ...
      u = 0; j = 0;
      for ( yb = ybi; yb != &Ultab_TO[reste]; yb++,j++) 
      {
       u |= (ptul[ind[t]] >> mod[t++] << j)&*yb;
      }
      ptu[nbc] = u;
